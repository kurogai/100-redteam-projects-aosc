...
def attack_ssh(host, user, password, port=22, timeout = 5)
  begin
    ...
    puts "[!] The remote '#{host}' has disconnected unexpectedly | #{host} : #{user} : #{password}"
  rescue Net::SSH::ConnectionTimeout
    puts "[!] The host '#{host}' not alive! | #{host} : #{user} : #{password}"
  rescue Net::SSH::Timeout
    puts "[!] The host '#{host}' disconnected/timeouted unexpectedly! | #{host} : #{user} : #{password}"
  rescue Errno::ECONNREFUSED
    puts "[!] Incorrect port #{port} for #{host} | #{host} : #{user} : #{password}"
  rescue Net::SSH::AuthenticationFailed
    puts "[!] Wrong Password: #{host} | #{user}:#{password} | #{host} : #{user} : #{password}"
  rescue Net::SSH::Authentication::DisallowedMethod
    puts "[!] The host '#{host}' doesn't accept password authentication method. | #{host} : #{user} : #{password}"
  rescue Errno::EHOSTUNREACH
    puts "[!] No route to host: '#{host}' | #{host} : #{user} : #{password}"
  rescue Errno::ECONNRESET
    puts "[!] Connection reset by peer: '#{host}' | #{host} : #{user} : #{password}"
